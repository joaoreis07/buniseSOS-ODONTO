...
type BrandProps = {
  /** Usar sobre superfície escura (sidebar azul-marinho). */
  light?: boolean;
  logoOnly?: boolean;
  className?: string;
};
        ...
        <span className="min-w-0 leading-tight">
          ...
          <span
            className={`block text-[10px] font-semibold uppercase tracking-[0.16em] ${
              light ? "text-brand-500" : "text-primary"
            }`}
          >
            Odonto
          </span>
...
export function Pill({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-200 bg-brand-50 px-2.5 py-1 text-[11px] font-medium tracking-wide text-primary">
      {children}
    </span>
  );
}
