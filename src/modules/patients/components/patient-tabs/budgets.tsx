...
export function PatientBudgetsTab({ patient, canManage }: { patient: PatientClientDTO; canManage: boolean }) {
  ...
  return <div className="space-y-3">
    <div className="flex items-center justify-between"><div><p className="font-medium">Orçamentos</p><p className="text-sm text-muted-foreground">Propostas e decisões comerciais.</p></div>{canManage && <Button asChild size="sm" className="rounded-lg"><Link href={`/app/budgets?patientId=${patient.id}&new=1`}><FilePlus2 className="mr-1 size-3.5" />Novo orçamento</Link></Button>}</div>
