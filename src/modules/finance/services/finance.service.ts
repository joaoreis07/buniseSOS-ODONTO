...
export async function listReceivables(companyId: string, filters?: { patientId?: string; receivableId?: string; status?: string; query?: string }) {
  ...
  return rows.map((row) => ({
    ...
    items: row.items.map((i) => ({ id: i.id, description: i.description, code: i.code, total: i.total.toFixed(2), teeth: i.teeth.map((t) => t.toothNumber), professionalName: i.professional?.name ?? null })),
